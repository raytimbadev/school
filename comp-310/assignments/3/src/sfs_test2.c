...
main(int argc, char **argv)
{
    ...
    for (i = 0; i < 2; i++) {
        for (j = i + 1; j < 2; j++) {
            if (fds[i] == fds[j]) {
                fprintf(stderr,
                        "Warning: the file descriptors probably shouldn't be "
                        "the same?\n");
            }
        }
    }
    ...
    for (i = 0; i < 2; i++) {
        if (sfs_fclose(fds[i]) == 0) {
            fprintf(stderr,
                    "Warning: closing already closed file %s\n",
                    names[i]);
        }
    }
    ...
    for (j = 0; j < strlen(test_str); j++) {
        for (i = 0; i < nopen; i++) {
            ...
            if (ch != test_str[j]) {
                fprintf(stderr,
                        "ERROR: Read wrong byte from %s at %d (%d,%d)\n",
    ...
    if (fds[0] >= 0) {
        for (i = 0; i < 100000; i++) {
            ...
            if (x != sizeof(fixedbuf)) {
                ...
                printf("If the emulated disk contains just over %d bytes, "
                        "this is OK\n",
    ...
    for (i = 0; i < nopen; i++) {
        ...
        if (fds[i] >= 0) {
            ...
            for (j = 0; j < strlen(test_str); j++) {
                if (test_str[j] != fixedbuf[j]) {
                    fprintf(stderr,
                            "ERROR: Wrong byte in %s at position %d (%d,%d)\n",
                            names[i], j, fixedbuf[j], test_str[j]);
                    error_count++;
                    break;
                }
